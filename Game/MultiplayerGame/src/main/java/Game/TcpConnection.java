...
class TcpConnection {
    ...
    void sendUpdatedVersion(CharacterObj character) {
        try {
            ...
            oos.reset();
            oos.flush();
            System.out.println(character.id + character.xVel+character.yVel);

//            if (character.id == 0 && character.xVel == 5){
//            System.out.println(sync + "id "+character.id+" velocity x " +character.xVel + " velocity y " + character.yVel);
//            sync++;
//            }
//
        } catch (IOException | JAXBException e) {
